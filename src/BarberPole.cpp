...
BarberPole::BarberPole(BRect pRect, const char *pName, uint32 resizingMode, uint32 flags, TBarberPoleDirection pDirection)
...
void BarberPole::SetActive(bool bActive) {
	if(bActive)
		Start();
	else
		Stop();
...
int32 BarberPole::spinningThread(void *data){
	...
	while(1==1){
		...
		if(lBarberPole->direction == bpdRightToLeft){
